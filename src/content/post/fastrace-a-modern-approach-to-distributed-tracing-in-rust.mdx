...
[dependencies]
tracing = { version = "0.1", features = ["log"] }  # Required for log compatibility

# In an application's Cargo.toml
[dependencies]
tracing = "0.1"
tracing-log = "0.2"  # Required bridge for log crate compatibility
```
...
Distributed tracing requires propagating context across service boundaries, but `tokio-rs/tracing` leaves this largely as an exercise for the developer. For example, this is [tonic official example](https://github.com/hyperium/tonic/blob/master/examples/src/tracing/server.rs) demonstrating how to trace gRPC service:
...
```rust
// This integration allows automatically associating your logs with
// trace spans, providing correlation without requiring using different
// logging macros.
...
`fastrace` represents a modern approach to distributed tracing in Rust - one that prioritizes simplicity, ecosystem compatibility, and performance. The most significant advantages of `fastrace` are:
